...
option_list <- list(
  ...
  make_option("--out", default = "results/tables/pseudobulk_genomewide_limma_voom.tsv.gz"),
  make_option("--summary-out", dest = "summary_out", default = "results/tables/pseudobulk_genomewide_limma_voom_summary.tsv"),
...
run_one <- function(counts, gene_info, metadata, manifest, state, case_group, control_group, min_donors, min_cells) {
  ...
  y <- calcNormFactors(y)
  voom_fit <- voom(y, design_info$design, plot = FALSE)
  fit <- eBayes(lmFit(voom_fit, design_info$design), robust = TRUE)
  table <- as.data.table(topTable(fit, coef = coefficient, number = Inf, sort.by = "P"), keep.rownames = "gene_id")
  table <- merge(gene_info, table, by = "gene_id", all.y = TRUE)
  setnames(table, old = c("logFC", "AveExpr", "P.Value", "adj.P.Val"), new = c("log2fc", "logcpm", "p_value", "fdr"), skip_absent = TRUE)
  table[, `:=`(
    contrast = paste0(case_group, "_vs_", control_group),
  ...
  list(result = table[, .(
    ...
    n_case, n_control, log2fc, logcpm, t, p_value, fdr, B, covariates, status
  )], summary = data.table(
    contrast = paste0(case_group, "_vs_", control_group),
    fine_cell_type = state,
    n_case = n_case,
...
message("Wrote limma-voom results: ", opt$out)
message("Wrote limma-voom summary: ", opt$summary_out)
