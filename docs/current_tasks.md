...
- 2026-06-14: Added Extra Trees, gradient boosting, and a conservative tree-ensemble average to test whether random forest was the practical ceiling. In 10-repeat donor-level CV, random forest remains best by MAE (14.20), while the tree ensemble is close by MAE (14.27) and slightly stronger on RMSE/R2/Spearman (RMSE 17.64, R2 0.107, Spearman 0.332). Gradient boosting is competitive but less stable by MAE (14.48). Current conclusion: random forest is still the safest headline model for MAE; tree ensemble is the best multi-metric challenger.
