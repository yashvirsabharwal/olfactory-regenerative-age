...
- 2026-06-22: Added and ran `make scvi-embedding-claim-gates`, which converts the full 4M, 250k seed, and 100k lineage scVI validation tables into publication-facing latent claim gates. The full 4M reduced model remains primary (`4,028,275` cells, 10 latent dimensions, fine-label purity `0.877`, coarse-label purity `0.975`), while smaller embeddings are sensitivity anchors. Basal and mature-OSN marker continuity are supported; immature-OSN, progenitor, immune, and sustentacular latent-mechanism wording remains guarded.
