...
| R2: Cross-Cohort Validation Atlas | in progress | Every credible public olfactory/nasal/airway aging or disease dataset is classified as direct, mapped, marker-only, context-only, or blocked; eligible datasets are processed through a shared adapter. | `results/tables/external_candidate_matrix.tsv`, `docs/external_validation_final_search.md`, `docs/gse184117_label_request.md`, `configs/external_datasets.yaml`, `results/tables/external_validation_evidence.tsv`. | Send/log GSE184117 label request, then promote any eligible candidate to adapter work only if the matrix class supports it. | Stronger external support if concordance survives harmonization. |
