...
| Module augmentation | Gate is complete when predictive gain and overlapping intervals reported. | Main text or supplement as biological annotation, not primary performance claim. | Keep as modest annotation/support. |
...
| AD/PD ORA projection | Gate is complete when all projection, matched-context, uncertainty, and label-permutation guardrails reported. | Main text exploratory section or supplement. | Keep exploratory only. |
...
| External Validation | needs work | Validation lead | GSE184117 labels are requested/recovered or documented unavailable; GSE151973 context is assessed; one final public-data search is logged; all evidence is classified as direct, mapped, marker-only, context-only, or blocked. | Current evidence: embedded external search and label-request logs below, `results/tables/external_validation_evidence.tsv`, `results/tables/external_scanvi_feature_concordance.tsv`, `results/tables/manuscript_table_external_validation_strength.tsv`. | Largest scientific weakness; small n and mixed concordance can limit acceptance. | At most guarded external support unless stronger data are recovered. | Send GSE184117 label request from project email, then update request outcome and manuscript limitations. |
...
| Reproducibility | needs work | Reproducibility lead | Heavyweight artifact locations/checksums are documented; stale workflow paths are removed; rerun profile exists; provenance reports zero missing non-deferred outputs. | Current non-deferred provenance exists in `results/reports/output_provenance.tsv`; Makefile is the active workflow; embedded artifact and rerun logs below document local checksums, `mia` remote checksums for the primary 4M artifacts, and rerun order. | Reviewer may question rerunnability of large latent/neighborhood results. | Engineering claim that outputs are reproducible from documented source state. | Move final 4M H5AD/model artifacts from `mia` to stable archival storage, add stable URIs, and rerun provenance before submission. |
| Manuscript Package | needs work | Manuscript lead | PDF compiles; all figures/tables resolve; citation keys pass; claims match ledger; supplement inventory is complete. | `manuscript/main.tex`, `manuscript/references.bib`, `results/reports/manuscript_package_check.md`, `docs/journal_acceptance_tracker.md`, `results/figures/manuscript_figure*.pdf`, `results/figures/extended_data_figure*.pdf`, `results/reports/publication_tables.md`. | Local TeX tooling is missing; unresolved figure/table paths would block submission. | Reviewable manuscript package once PDF compiles. | Build in a TeX-enabled environment, visually inspect PDF, and run final claim-language audit. |
| Submission Compliance | needs work | Submission lead | Target journal checklist is completed, cover letter is drafted, data/code availability is finalized, and reviewer-risk memo is written. | `docs/journal_acceptance_tracker.md`. | Administrative mismatch can delay or desk-reject an otherwise strong paper. | Submission-ready package after target-specific fields, archive URI, and author statements are finalized. | Pick target journal, add stable artifact URI, finalize author/funding/ethics/conflict fields, and tailor cover letter. |
...
| M2: Maximum External Validation Push | needs work | GSE184117 labels are requested/recovered or documented unavailable; GSE151973 context is assessed; final public-data search is logged; all validation evidence is classified. | Embedded external search and label-request logs below, updated evidence ledger, validation-strength table, manuscript limitation text. | External validation remains guarded unless stronger donor-level evidence is recovered. | Access to public metadata and any author response. |
...
| --- | --- | --- | --- |
| Request GSE184117 author labels or annotation files | draft ready | `docs/journal_acceptance_tracker.md` or equivalent logged note; tracker updated with request date and outcome. | Labels integrated, authors decline, no response after a documented waiting period, or files are unavailable. |
| Reassess GSE151973 as context-only bulk/deconvolution evidence | complete | Note in `docs/journal_acceptance_tracker.md`; evidence ledger updated if useful. | It either clarifies marker specificity as context-only or is documented as not useful. |
...
| Provenance refresh | current | `PYTHON=.venv/bin/python make output-provenance` reports zero missing non-deferred outputs after final figures/tables. | Deferred artifacts are covered by `docs/journal_acceptance_tracker.md`. |
...
| Figure resolution/path check | All main and extended-data figure PDFs exist and are referenced correctly. | pass in `results/reports/manuscript_package_check.md` |
| Publication table inventory | `results/reports/publication_tables.md` lists all manuscript-ready tables. | current |
| Citation check | Every citation key in `manuscript/main.tex` exists in `manuscript/references.bib`. | pass in `results/reports/manuscript_package_check.md` |
| PDF build | `make manuscript` succeeds in a TeX-enabled environment. | blocked locally by missing TeX |
| Supplement inventory | All supplementary tables/figures are named, described, and claim-gated. | drafted in `docs/journal_acceptance_tracker.md` |
...
## Embedded Submission Logs

### External Search And Label Request

Final public-data search verdict from 2026-06-23: no public dataset found is stronger than `GSE184117` for direct donor-level human olfactory aging validation. `GSE184117` remains the primary direct but small-n external target; `GSE151973` is marker/context only; `GSE290883`, `GSE290884`, `GSE235330`, `GSE324335`, `GSE151346`, and broad spatial resources are context-only or blocked for the primary ORA claim.

`GSE184117` label request state: draft ready, not yet sent from project email. GEO contact recorded: Bradley Goldstein, Duke University, `bradley.goldstein@duke.edu`. Requested materials are per-cell labels, barcode/sample mapping, donor/sample phenotype metadata, annotation/QC notes, and redistribution/citation guidance. Claim rule: do not promote `GSE184117` beyond small-n supportive/context evidence unless labels or defensible mapping support donor-level ORA feature replication.

### Artifact And Rerun Package

Large artifacts are intentionally not committed. Publication-critical local artifacts include the 250k scVI atlas, lineage 100k scVI atlas, GSE184117 mapped/scANVI objects, and scANVI reference model. Publication-critical `mia` artifacts include `gateway_hvg3003_4m.h5ad`, `gateway_scvi_full_4m_reduced.h5ad`, `results/models/gateway_scvi_full_4m_reduced/model.pt`, `gateway_scvi_stratified_250k_seed23.h5ad`, and its model weights. Before submission, replace compute paths with stable archive URIs and refresh SHA-256 checksums.

Recommended rerun order: local refresh runs Ruff/tests, external evidence, model card, feature interpretation, NDD guardrails, publication tables, manuscript figures, and output provenance. Heavy latent refresh runs reduced 4M substrate, full 4M reduced scVI, validation, lineage Milo-style full/matched, program and age-bin summaries, edgeR parity, MiloR subset sensitivity, then scVI claim gates. Final submission build requires `make publication-tables`, `make manuscript-figures`, `make manuscript-check`, `make output-provenance`, and `make manuscript` in a TeX-enabled environment.

### Submission Package

Current submission state: manuscript source and figures/tables are present, claims are conservative, external validation is exhausted but label request is pending, reproducibility still needs durable artifact archive URI, and author/admin fields remain pending.

Required admin fields: target journal/article type, author list, corresponding author, CRediT contributions, funding, conflicts, ethics/consent reuse language, acknowledgements, reviewer suggestions, preprint decision, and license confirmation.

Data/code availability draft: Gateway atlas source DOI `10.64898/2026.06.10.731272`; configured CELLxGENE collection ID `8b35aa1f-6bcf-4a51-abc3-a3f336a44ae6`; public validation/context datasets include `GSE184117`, `GSE151973`, and context-only candidates listed in `configs/external_datasets.yaml`; code at `https://github.com/yashvirsabharwal/olfactory-regenerative-age`; final release tag/commit and artifact archive URI must be inserted before submission.
