...
| Manuscript Package | needs work | Manuscript lead | PDF compiles; all figures/tables resolve; citation keys pass; claims match ledger; supplement inventory is complete. | `manuscript/main.tex`, `manuscript/references.bib`, `results/reports/manuscript_package_check.md`, `docs/submission_package.md`, `results/figures/manuscript_figure*.pdf`, `results/figures/extended_data_figure*.pdf`, `results/reports/publication_tables.md`. | Local TeX tooling is missing; unresolved figure/table paths would block submission. | Reviewable manuscript package once PDF compiles. | Build in a TeX-enabled environment, visually inspect PDF, and run final claim-language audit. |
