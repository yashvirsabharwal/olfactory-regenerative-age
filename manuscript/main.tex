...
As a compositional robustness check, we fit donor-level CLR models for fine-cell-state composition with age as a continuous predictor and sex, chemistry, collection method, and total cell yield as covariates where estimable. Ten primary cell states were significant at FDR $<0.10$ and remained directionally stable in strict-threshold and single chemistry/collection sensitivity analyses. The strongest positive age-associated compositional shifts included quiescent HBC, suprabasal, multiciliated, classical monocyte, proliferating secretory, and MV type 2 states, while cytotoxic CD8, cDC1, pre-dysfunctional, and antigen-presenting states were negative. These composition-aware directions were concordant with the ordinary ORA CLR age-association table for the significant stable rows.
...
Single-run composition-only XGBoost achieved mean absolute error (MAE) of 13.73 years and Spearman $r=0.41$ in donor-level cross-validation \citep{xgboost2016}. Repeated CV benchmarks showed that tree and boosted models consistently outperformed the null model, but gains were modest. In 10-repeat composition-only CV, tree ensemble, XGBoost, and random forest were near MAE 14.19 years. Module-augmented repeated benchmarks showed CatBoost, tree ensemble, histogram gradient boosting, and XGBoost near MAE 14.08 to 14.10 years, with overlapping intervals. Shuffled-age permutation tests supported that observed performance was above null expectations, with empirical $p=0.0196$ at 50 permutations for the top tested models. Model-card tables report the backend package, version, and fallback status for each row; the current core ORA rerun used native backends for XGBoost, LightGBM, CatBoost, and scikit-learn models.
...
Negative-control analyses showed that technical covariates are non-negligible and should remain visible in the claim boundary. A technical-only ridge baseline achieved MAE 14.98 years, yield-only ridge MAE 15.69 years, and null mean CV MAE 15.64 years, while a simple biological-feature ridge control was weaker (MAE 17.21 years). However, the best observed repeated ORA candidate remained better than these simple controls (CatBoost MAE 14.08 years). Age-shuffled biological labels within sex, chemistry, and collection-method strata were worse (mean MAE 19.94 years; 95\% interval 17.45 to 22.18). Across fitted ORA score tables, all technical covariates explained nonzero but limited residual ORA acceleration variance, reaching 0.15 $R^2$ for the most technically sensitive model and 0.10 for CatBoost. We therefore report ORA as a biological-feature model with measurable technical dependence, not as a technical-covariate-free clock.
...
A public-data exhaustion search identified additional olfactory disease and smell-loss context datasets, including the public GSE302937 intranasal olfactory epithelium biopsy single-cell dataset for Alzheimer-stage disease context, but no stronger public independent donor-level human olfactory epithelial healthy-aging sc/snRNA validation cohort. These datasets are therefore tracked as context or mapped stress tests rather than replacements for GSE184117 as the direct external validation target.
...
\subsection{Genome-wide DE and latent-neighborhood analyses remain audited extensions}
...
Latent-space analysis was also gated. The CELLxGENE export exposes only \texttt{X\_umap}, not author scVI/scANVI coordinates. We therefore installed scanpy, scvi-tools, and torch, then used a chunked reduced-H5AD workflow to train scVI on all 4,028,275 cells with a 3,003-gene HVG/marker feature set, \texttt{sample\_id} as the batch key, and \texttt{flex\_version}, \texttt{device\_guided}, and \texttt{sex} as categorical covariates \citep{scanpy2018,scvi2018,scvitools2022}. The full 4M reduced model produced finite 10-dimensional \texttt{X\_scvi} coordinates, fine-label purity 0.877, and coarse-label purity 0.975. Stratified 250,000-cell seed models and a lineage-focused 100,000-cell model were retained as sensitivity anchors. Cross-run marker gates supported basal and mature-OSN continuity, while immature-OSN, progenitor, immune, and sustentacular latent-mechanism language remained guarded.

Using the full 4M reduced scVI latent space, we ran publication-scale Python Milo-style donor-level neighborhood differential abundance with 20,000 stratified seed neighborhoods and 100 nearest cells per neighborhood. The full lineage-focused run identified 5,613 age-associated neighborhoods at FDR $<0.10$, while strict FLEX v2/device matching reduced the lineage result to one significant Early iOSN neighborhood. Exact-neighborhood edgeR parity supported the signed age direction of the Python workflow (signed-effect Spearman 0.916 for all-donor lineage and 0.924 for matched lineage), and the single matched Python-significant Early iOSN neighborhood was also edgeR-significant. Official MiloR was then run as an independent subset sensitivity on 100,000 lineage cells and 75,000 matched lineage cells from the same full 4M latent space \citep{milo2022}. MiloR confirmed broad age-associated lineage-neighborhood structure, but its dominant matched signals were HBC, suprabasal, and sustentacular rather than a direct Early iOSN replication. The manuscript-safe interpretation is therefore broad lineage-neighborhood remodeling, with Early iOSN depletion treated as a narrow exact-neighborhood subclaim supported by edgeR, age-bin directionality, and curated program enrichment.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{manuscript_figure6_de_latent.pdf}
  \caption{\textbf{Genome-wide DE and latent-space audits.} Disease DE is sensitive to method and matched-context choices, with sentinel categories explicitly audited. Full 4M reduced scVI supports a publication-scale latent substrate; Milo-style, edgeR, and official-MiloR sensitivity analyses support guarded lineage-neighborhood interpretation rather than trajectory claims.}
  \label{fig:de_latent}
\end{figure}
...
\subsection{Compositional age modeling}

Formal compositional age modeling used donor-level fine-cell-state counts with a 0.5 pseudocount CLR transform. Each cell-state CLR response was tested against continuous age, with coefficients reported per 10 years. Covariates included sex, chemistry, collection method, site, and log10 donor cell yield when those covariates had estimable variation. The primary scenario used healthy donors with valid age and ORA training eligibility. Sensitivity scenarios used the strict cohort-threshold subset and the largest single chemistry/collection stratum. Cell states with insufficient nonzero donor coverage were excluded. Results were compared against the ordinary ORA CLR age-association table by mapped feature name.
...
ORA models predicted chronological age from donor-level healthy features only. Model families included regularized linear models, random forests, extra trees, gradient boosting, histogram gradient boosting, XGBoost \citep{xgboost2016}, LightGBM, CatBoost, and conservative ensembles. Cross-validation was donor-level. Repeated CV summarized performance variability across repeat splits. Shuffled-age permutation tests estimated empirical null performance. Nested tuning used inner donor-level CV within outer folds for selected boosted models. Calibration diagnostics regressed predicted ORA on chronological age and compared raw and recalibrated MAE.

\subsection{Negative controls and confounding checks}

Negative controls used the same healthy donor set as ORA training. Deterministic ridge baselines compared biological features, technical covariates only, total cell yield only, and a fold-wise mean-age null. Age-shuffle controls permuted age labels within sex, chemistry, and collection-method strata before donor-level CV. Disease-label prediction was recorded as not applicable because the primary training set contains healthy age-known donors only. Technical explainability checks regressed fitted ORA and ORA-acceleration scores against total cell yield, chemistry, collection method, site, and their combined technical design, with age included for raw ORA-score models.
...
\subsection{Latent-space audit and neighborhood validation}

The public H5AD was audited for \texttt{.obsm} embeddings before trajectory work. Only \texttt{X\_umap} was present in the export. The full Gateway H5AD was reduced in contiguous row chunks to a 3,003-gene HVG/marker feature set and used to train scVI across all 4,028,275 cells. Stratified 250,000-cell seed models and a lineage-focused 100,000-cell model used the same marker-preserving strategy as sensitivity anchors. Validation covered dimensions, finite values, metadata coverage, nearest-neighbor label purity, technical mixing, and marker continuity. A publication-facing comparison table classified the full 4M model as the primary latent substrate and smaller models as sensitivity anchors.

Full-scale Python Milo-style neighborhood differential abundance was run on the all-cell reduced scVI latent atlas. Neighborhoods were selected as 20,000 stratified seed neighborhoods with 100 nearest cells per neighborhood. Donor-level models tested logit neighborhood fraction as a function of age, adjusting for sex, chemistry, collection method, and donor yield where estimable. Matched FLEX v2/device sensitivity restricted the healthy donor set to the disease-sample technical context. Neighborhood memberships were exported for age-bin summaries and curated program scoring. The same Python neighborhoods were also exported as neighborhood-by-donor count matrices and tested with edgeR quasi-likelihood models as exact-neighborhood statistical parity. Official MiloR subset sensitivity used independent MiloR neighborhood construction on stratified lineage exports from the same full 4M latent space \citep{milo2022}. The Python workflow is therefore described as Milo-style, while official MiloR is reported as implementation sensitivity.
